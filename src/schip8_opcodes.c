...
void op_00FF(CHIP8* chip8, Display* display) {
	display_hi_res(display);
}

void op_00FE(CHIP8* chip8, Display* display) {
	display_low_res(display);
}

void op_00CN(CHIP8* chip8, Display* display, u8 N) {
...
void op_00FB(CHIP8* schip8, Display* display) {
	int screen_height = display->extended_resolution ? SUPER_CHIP8_HEIGHT : CHIP8_HEIGHT;
	int screen_width = display->extended_resolution ? SUPER_CHIP8_WIDTH : CHIP8_WIDTH;
...
void op_00FC(CHIP8* chip8, Display* display) {
	int screen_height = display->extended_resolution ? SUPER_CHIP8_HEIGHT : CHIP8_HEIGHT;
	int screen_width = display->extended_resolution ? SUPER_CHIP8_WIDTH : CHIP8_WIDTH;
